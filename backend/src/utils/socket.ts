...
export const initializeSocket = (httpServer: HttpServer) => {
	...
	io.use(async (socket, next) => {
		const token = socket.handshake.auth.token // this is what user will send from frontend from client
		if (!token) return next(new Error("Socket Authenticated Error"));
	...
	// this "connection" event name si special and should be written like this
	...
	io.on("connection", (socket) => {
		...
		// notify other that user is just disconnected
